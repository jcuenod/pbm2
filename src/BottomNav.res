...
let make = (~index, ~onSelect) => {
  ...
  <nav
    className="bg-white dark:bg-stone-900 border-t border-stone-200 dark:border-stone-800 p-2 relative"
  >
    <div className="flex justify-around">
      ...
      />
      {items
      ->Belt.Array.mapWithIndex((i, (label, icon)) =>
        <button
          key={label}
          ...
          className={"relative w-[24%] pt-1 pb-1 z-10 flex flex-col items-center text-sm transition-transform duration-200 ease-in-out " ++ if (
            i == index
          ) {
            "text-teal-600 scale-100"
          } else {
            "text-stone-500 scale-90"
          }}
        >
          {icon}
