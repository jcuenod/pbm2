...
let make = (~totalPages: int, ~currentPage: int, ~onPageChange: int => unit) => {
  let windowSize = 5
  let startPage = if totalPages <= 0 {
    0
  } else {
    let startCandidate = currentPage - 2
    let fromLower = if startCandidate < 0 {
      0
    } else {
      startCandidate
    }
    let maxStart = if totalPages - windowSize > 0 {
      totalPages - windowSize
    } else {
      0
    }
    if fromLower > maxStart {
      maxStart
    } else {
      fromLower
    }
  ...
  let navButtonClass = enabled =>
    "h-10 w-10 rounded text-sm font-semibold transition-all active:scale-95 flex items-center justify-center border border-stone-200 dark:border-stone-700 " ++ if (
      enabled
    ) {
  ...
  let slotClass = isActive =>
    "h-10 min-w-[2.75rem] rounded text-sm font-semibold transition-all active:scale-95 flex items-center justify-center " ++ if (
      isActive
    ) {
  ...
  <div className="flex items-center justify-center gap-2">
    ...
    ->Belt.Array.mapWithIndex((idx, slot) =>
      ...
      | Some(page) =>
        ...
        <button
          key={idx->Int.toString} className={slotClass(isActive)} onClick={_ => goToPage(page)}
        >
