...
let make = (~collapsed, ~onClick, ~reference="Matthew 1", ~onNext=?, ~onPrev=?) => {
  ...
  <div
    ...
    className={"bg-white dark:bg-stone-900 shadow flex items-center justify-center cursor-pointer transition-all duration-300 relative " ++ (
      collapsed ? "h-0 py-0 opacity-0 overflow-hidden pointer-events-none" : "h-12 py-3"
    )}
  >
    <div className="flex items-center gap-4">
      ...
      <h2 className="font-semibold text-lg select-none"> {React.string(reference)} </h2>
